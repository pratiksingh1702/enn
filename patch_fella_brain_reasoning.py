                ...
                if self.wave_engine.determine_spectron_type(self.substrate.neurons[target_id]) == "hot":
                    ...
                    for w in words:
                        ...
                        if n.id != target_id and n.text != "is":
                            # Look at its synapses for a catalyst bridge
                            for syn_id, weight in n.synapses.items():
                                syn_n = self.substrate.neurons[syn_id]
                                if self.wave_engine.determine_spectron_type(syn_n) == "catalyst":
                                    for ans_id, ans_weight in syn_n.synapses.items():
                                        ans_n = self.substrate.neurons[ans_id]
                                        if ans_n.id != n.id:
                                            response_text = f"{n.text} {syn_n.text} {ans_n.text}"
                                            found_answer = True
                                            print(f"[REASONING] Retrieved from Tier 3 Standing Wave: {response_text}")
                                            break
